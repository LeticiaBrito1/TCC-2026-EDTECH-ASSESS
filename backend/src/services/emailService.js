...
const buildVerificationEmail = (toEmail, toName, verificationUrl) => ({
  from: `"EdTech Assess" <${env.smtpFrom}>`,
  to: toEmail,
...
export const sendLoginCodeEmail = async ({ toEmail, toName, code }) => {
  if (!transporter) {
    console.log("\n========================================");
    console.log("[email] SMTP não configurado — código de login:");
  ...
  try {
    ...
    console.log(`[email] Código de login enviado para ${toEmail}`);
    return { sent: true, messageId: info.messageId };
  } catch (smtpError) {
    console.error(`[email] Falha ao enviar código: ${smtpError.message}`);
    console.log("\n========================================");
...
export const sendPasswordResetEmail = async ({ toEmail, toName, token }) => {
  ...
  if (!transporter) {
    console.log("\n========================================");
    console.log("[email] SMTP não configurado — link de redefinição de senha:");
    console.log(`[email] Para: ${toEmail} (${toName})`);
  ...
  try {
    ...
    console.log(`[email] Link de redefinição enviado para ${toEmail} (${info.messageId})`);
    return { sent: true, messageId: info.messageId };
  } catch (smtpError) {
    console.error(`[email] Falha ao enviar redefinição: ${smtpError.message}`);
    console.log("\n========================================");
...
export const sendVerificationEmail = async ({ toEmail, toName, token }) => {
  ...
  if (!transporter) {
    // Sem SMTP configurado: exibe o link no console (modo desenvolvimento)
    console.log("\n========================================");
    console.log("[email] SMTP não configurado — link de verificação:");
    console.log(`[email] Para: ${toEmail} (${toName})`);
  ...
  try {
    const info = await transporter.sendMail(
      buildVerificationEmail(toEmail, toName, verificationUrl)
    );

    const previewUrl = nodemailer.getTestMessageUrl(info);
    if (previewUrl) console.log(`[email] Preview (Ethereal): ${previewUrl}`);

    console.log(`[email] E-mail enviado para ${toEmail} (${info.messageId})`);
    return { sent: true, messageId: info.messageId };
  } catch (smtpError) {
    console.error(`[email] Falha no SMTP: ${smtpError.message}`);
    console.log("\n========================================");
    console.log("[email] FALLBACK — link de verificação (SMTP falhou):");
    console.log(`[email] Para: ${toEmail} (${toName})`);
    console.log(`[email] Link: ${verificationUrl}`);
    console.log("========================================\n");
    return { sent: false, error: smtpError.message };
